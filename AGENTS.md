...
### 5.9 同一文件的多次编辑必须串行（工具坑）
**在同一条消息里并行发多个 Edit 改同一个文件，只有其中一个会真落盘**，其余被
「基于旧快照的写入」覆盖 —— 而工具**每个都返回 "Successfully edited"**，极具迷惑性。

→ 一次只发一个 Edit；改完立刻 `grep` 关键常量复核（例：`setHSL` / `#78838a` /
`receiveShadow`），**不要相信 Edit 的成功回执**。第 12 轮为此白跑了 3 张截图。

### 5.10 「看着不对」先放大看，别急着猜
3D 里的"锯齿 / 毛刺 / 怪色块"，先**裁剪放大**（PIL crop + 3× LANCZOS）再判断。
第 12 轮那块"远处地面的锯齿"，放大后露出**受光面/背光面** —— 一眼排除
"抗锯齿伪影 / z-fighting"，锁定为「阴影出框被 clamp」。
...
- 默认视角 = **第三人称环视**（透视轨道相机，可旋转/缩放/平移），
  默认取景 `az=25 / el=36 / dist=1.10`；另有 `?mode=top` 正交图纸视角、
  `?mode=walk` 第一人称漫游
- 地面按用途**分区铺装**（`floorKind()` 按房间名关键字分类：卧室系木地板 / 阳台与卫生间
  瓷砖 / 其余毛坯）：纹理由 canvas 程序化生成，零外部图片资源；
  UV 取**世界米数**，故各房间砖板同尺度、相邻房间接得上
- 每个房间挂 **「房名 + 面积」标注**（canvas 中文贴图 + billboard sprite）：
  锚点取面积加权质心（凹多边形回退 bbox 中心），尺寸按房间面积与视角双自适应；
  `?labels=0` 关闭，第一人称模式自动隐藏
- 套型按 **17/26 层**渲染外部环境：天空穹顶 + 地坪两级网格（5m/50m）+ 两圈城市剪影 +
  楼身（下方极淡半透明壳）+ 上方 `FloorRings` 楼层环线（地坪 −48m → 本层 → 塔顶），
  俯视模式下自动关闭
- 外部环境的观感已返修过一轮（详见 `RUN-LOG.md` 第 12 轮）：去掉了「竖条纹幕布」
  「纱柱」「地面影块」三处缺陷，城市剪影改为不透明分级配色
...
5. LDK 内的**厨房区未单独铺瓷砖**——LDK 是"客厅+餐厅+厨房"的合并房间（52.61 m²），
   厨房区在几何上切不出来，故整间按木地板处理；要切分需引入多边形布尔运算。
